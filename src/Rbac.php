...
namespace Peter\Permission\Rbac;


use Perter\Permission\rbac\model\PermissionCategory;
use Perter\Permission\rbac\model\Role;
use Peter\Permission\nestedsets\NestedSets;
use Peter\Permission\rbac\model\Permission;


use Peter\Permission\Exceptions\Exception;
use Peter\Permission\rbac\model\UserRole;
use think\Db;
use think\db\Query;
use think\db\Where;
...
//            $token = Request::header('token');
        ...
        if (isset($permissionList[$path]) && !empty($permissionList[$path])) {
            //根据请求方式判断权限
            $methods = $permissionList[$path]['method'];//获取请求方式
    ...
    {
        $token = md5($prefix . $this->randCode(64) . $this->saltToken . time());
        $freshTOken = md5($prefix . $this->randCode(64) . $this->saltToken . time());
